...
export default function DocumentListTable() {
  ...
  const uniqueDocumentStatuses = useMemo(() => {
    return Array.from(
      new Set(
        documentList.map((doc) => doc.documentStatus || "").filter(Boolean)
      )
    ).sort();
  }, []);
  ...
  const [documentStatusSearch, setDocumentStatusSearch] = useState("");
  const filteredDocumentStatuses = useMemo(() => {
    if (!documentStatusSearch) return uniqueDocumentStatuses;
    return uniqueDocumentStatuses.filter((status) =>
      status.toLowerCase().includes(documentStatusSearch.toLowerCase())
    );
  }, [documentStatusSearch, uniqueDocumentStatuses]);
                    ...
                    <PopoverContent className="w-[300px] p-0">
                      <div className="p-2">
                        <Input
                          ...
                          onChange={(e) =>
                            setRequestorNameSearch(e.target.value)
                          }
                          className="mb-2"
                        />
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        ...
                        className="w-full justify-between cursor-pointer"
                      >
                        {filters.documentStatus || "Select document status..."}
                        <SearchIcon className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-[300px] p-0">
                      <div className="p-2">
                        <Input
                          placeholder="Search document status..."
                          value={documentStatusSearch}
                          onChange={(e) =>
                            setDocumentStatusSearch(e.target.value)
                          }
                          className="mb-2"
                        />
                        <div className="max-h-[200px] overflow-auto">
                          {filteredDocumentStatuses.length === 0 ? (
                            <div className="py-6 text-center text-sm text-muted-foreground">
                              No document status found.
                            </div>
                          ) : (
                            <>
                              <Button
                              ...
                              {filteredDocumentStatuses.map((status) => (
                                <Button
                                  key={status}
                                  variant="ghost"
              ...
              <SheetFooter className="flex flex-row gap-2 w-full items-center justify-between">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    className="cursor-pointer"
                  >
                    <CheckCircle2 className="mr-2 h-4 w-4r" />
                    Apply Filters
                  </Button>
                </div>
              </SheetFooter>
